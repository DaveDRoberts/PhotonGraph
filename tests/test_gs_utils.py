...
@pytest.fixture
def state_vector_n3_d3_lme_not_ru():
    """Returns: A three qutrit LME state which is not RU."""

    n = 3
    d = 3
    vector = np.array([0.0272420087 - 0.1905122306j,
                       ...
                       -0.102339975 + 0.1629833284j], dtype='complex128')

    return StateVector(n, d, vector)


@pytest.fixture
def state_vector_n4_d4_ghz():
    """Returns: A four ququart GHZ state."""

    n = 4
    d = 4
    vector_temp = np.zeros(256, dtype='complex128')
    vector_temp[0] = 1
    vector_temp[-1] = 1
    vector = 1 / np.sqrt(2)*vector_temp

    return StateVector(n, d, vector)


@pytest.fixture
def state_vector_n3_d4():
    """Returns: A three ququart state that is a graph state."""

    n = 3
    d = 4
    vector = np.array([(0.125 + 0j),(0.125 + 0j),(0.125 + 0j),(0.125 + 0j),
                       (0.125 + 0j),(-0.125 + 0j),(0.125 + 0j),(-0.125 + 0j),
                       (0.125 + 0j),(0.125 + 0j),(0.125 + 0j),(0.125 + 0j),
                       (0.125 + 0j),(-0.125 + 0j),(0.125 + 0j),(-0.125 + 0j),
                       (0.125 + 0j),(0.125 + 0j),(0.125 + 0j),(0.125 + 0j),
                       (0 + 0.125j),(-0.125 + 0j),(0 - 0.125j),(0.125 + 0j),
                       (-0.125 + 0j),(0.125 + 0j),(-0.125 + 0j),(0.125 + 0j),
                       (0 - 0.125j),(-0.125 + 0j),(0+ 0.125j),(0.125 + 0j),
                       (0.125 + 0j),(0.125 + 0j),(0.125 + 0j),(0.125 + 0j),
                       (-0.125 + 0j),(-0.125 + 0j),(-0.125 + 0j),(-0.125 + 0j),
...
@pytest.fixture
def state_vector_n4_d2():
    """Returns: A four qubit graph state"""
...
@pytest.mark.parametrize("state_vector, check_type, exp_result", [
    (state_vector_n3_d3_with_gp, "LME", True),
    (state_vector_n3_d3_with_gp, "RU", True),
    (state_vector_n3_d3_ru_not_gs, "RU", True),
    (state_vector_n3_d3_lme_not_ru, "RU", False),
    (state_vector_n3_d3_lme_not_ru, "LME", True),
    (state_vector_n4_d4_ghz, "RU", False)])
def test_state_check(state_vector, check_type, exp_result):
    assert state_check(state_vector, check_type) == exp_result


@pytest.mark.parametrize("state_vector, exp_result", [
    (state_vector_n3_d3_with_gp, {(1, 2): 2, (0, 1): 1, (0, 1, 2): 2}),
    (state_vector_n3_d3_ru_not_gs, AssertionError),
    (state_vector_n4_d4_ghz, AssertionError),
    (state_vector_n3_d4, {(1, 2): 2, (0, 1): 1, (0, 1, 2): 3}),
    (state_vector_n4_d2, {(2, 3): 1, (1, 3): 1, (1, 2): 1, (0, 1): 1})])
def test_gs_from_sv(state_vector, exp_result):
    assert gs_from_sv(state_vector) == GraphState
